...
local function window_size_read()
	...
	if window_size_cache_file ~= nil then
		_, _, cols, rows = string.find(window_size_cache_file:read(), "(%d+),(%d+)")
...
local current_window_size = window_size_read()
local current_font_size = font_size_read()

local function on_window_resized(window, pane)
	local tab_size = pane:tab():get_size()

	cols = tab_size["cols"]
	rows = tab_size["rows"]

	current_window_size = { cols = cols, rows = rows }
...
local function on_decrease_font_size(window, pane)
	window:perform_action(wezterm.action.DecreaseFontSize, pane)
	current_font_size = current_font_size * 0.9

	local font_size_cache_file = assert(io.open(font_size_cache_path, "w"))
	font_size_cache_file:write(string.format("%f", current_font_size))
	font_size_cache_file:close()

	on_window_resized(window, pane)
end

local function on_increase_font_size(window, pane)
...
c.font_size = current_font_size
c.initial_rows = current_window_size.rows
c.initial_cols = current_window_size.cols
...
colors, _ = wezterm.color.load_scheme(expandPath("~/.config/wezterm/gruvbox-dark-hard.toml"))
...
wezterm.on("decrease-font-size", function(window, pane)
	on_decrease_font_size(window, pane)
end)

wezterm.on("increase-font-size", function(window, pane)
	on_increase_font_size(window, pane)
end)

wezterm.on("gui-startup", function(window)
	local tab, pane, window = mux.spawn_window({
		width = current_window_size.cols,
		height = current_window_size.rows,
	})
